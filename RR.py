  ...
  def RR(self, Array):
    ...
    while Count != 0:
      ...
      if not Overloading:
        ...
        for process in ReadyList:
          if(process == Executing) or (process.StartLine >= Total):
            continue
          process.PrintList.append("0")
          process.WaitTime += 1

      else:
        self.progressTable.loc[int(Executing.ProcessId), Total-1].configure({"background":'Red'})
        for process in ReadyList:
          if process != Executing:
            self.progressTable.loc[int(Executing.ProcessId), Total-1].configure({"background":'Grey'})
        self.interface.update()
      ...
      for process in Copy:
        for i in range(process.WaitTime + process.ExecutedTime + process.StartLine , Total):
          process.PrintList.append(" ")
