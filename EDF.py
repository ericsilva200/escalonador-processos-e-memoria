    ...
    while Count != 0:
        ...
        for process in ReadyList:
          if process.StartTime <= None:
            if Executing == None:
              ...
              if process.Deadline - (Total - process.StartTime) < Executing.Deadline - Total:
                Executing = process   
      ...
      if not Overloading:
        try:
          ...
          Executing.ExecutionTimePerQuantum += 1
          ...
          if Executing != None:
            ...
            for process in ReadyList:
              if process != Executing:
                self.progressTable.loc[int(Executing.ProcessId), Total-1].configure({"background":'Grey'})
            self.interface.update()

          if Executing.Deadline - (Total - Executing.StartTime) < 0:
              self.progressTable.loc[int(Executing.ProcessId), Total-1].configure({"background":'Blue'})
              self.interface.update()
          ...
          elif Executing.ExecutionTimePerQuantum == self.Quantum and self.Overload > 0:
            Executing.ExecutionTimePerQuantum = 0
            Overloading = True
        ...
        if Executing != None:
          ...
          self.interface.loc[int(Executing.ProcessId), Total-1].configure({"background":'Red'})
          for process in ReadyList:
            if process != Executing:
              self.progressTable.loc[int(Executing.ProcessId), Total-1].configure({"background":'Grey'})
          self.interface.update()
        ReadyList = np.delete(ReadyList, np.where(ReadyList == Executing))
        ReadyList = np.append(ReadyList, Executing)
